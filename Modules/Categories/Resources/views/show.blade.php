...
@section('breadcrumb')
    {{ Breadcrumbs::render($breadcrumbs['show'], $entity) }}
@endsection
@section('content')
    <div class="card">
        <div class="card-header bg-white header-elements-inline">
            <h6 class="card-title font-weight-bold">@lang($language_file.'.name'): {{ $entity->name }}</h6>
            <div class="header-elements">
{{--                @if($permissions['create'] == '' or Auth::user()->hasPermissionTo($permissions['create']))--}}
{{--                    <a href="{{ route($routes['create'], ['parent_id' => $entity->id, 'type' => Request::get('type')]) }}"--}}
...
@push('scripts')
    ...
    <script src="/cms/js/plugins/buttons.server-side.js"></script>
    {!! $dataTable->scripts() !!}
@endpush
